...
`tutorials/tabiclv2_regressor_colab.ipynb` (`E2E`) and its companion `tutorials/tabiclv2_regressor_artifact_inference_colab.ipynb` (`ARTIFACT-INFERENCE`) are declared under DIMER Notebook Specification 1.1 and are **standalone** (§3.6): generated by `tools/build_notebook.py`, each carries the public reference API module `src/tabicl_regressor_pipeline/api.py` verbatim, the model identity (`jingang/TabICL` at immutable revision `4dcd344ece2c00be9e831fdd35bed57b5ad83e19`), the snapshot manifest digest (`0db9cb538f114e79026bf08f45f41ad8dd7ad2de2aaca9a5ca8cd3bd9748ae7a`) and the exact `pyproject.toml` runtime pins, so the exported notebooks run without this repository or DIMER Workbench (parity enforced by `tests/test_notebook_parity.py`, `tests/test_companion_parity.py` and `tools/validate_release_assets.py`). The E2E default path loads scikit-learn's bundled diabetes table (no download), validates it with `validate_inputs` into an input manifest, conditions the pretrained model, evaluates it on a holdout and an independent test partition against `training_mean_baseline`, LightGBM and Random Forest, writes an `evaluation_report` (`sample-sanity`), optionally fine-tunes on CUDA (`RUN_FINE_TUNING`, off by default) and optionally scores an uploaded CSV, exports a DIMER-style serving bundle and proves it reloads from a fresh directory. The companion consumes a bundle from a separate execution (upload or `ARTIFACT_ZIP_PATH`), validates it before reconstruction, validates new unlabelled rows into an input manifest, predicts, and writes a `not-measurable` evaluation report. See `tutorials/README.md` for the registry, `docs/WEIGHTS.md` for the snapshot layout and `docs/release-verification.md` for the release gate.
